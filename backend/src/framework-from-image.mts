...
} from "./ve-execution/ve-execution-ssh-executor.mjs";
...
export class FrameworkFromImage {
  ...
  static async getAnnotationsFromImage(
    ...
    const annotations = await this.executeOnVeHost(
      veContext,
      scriptContent,
      image,
      tag,
  ...
  private static async executeOnVeHost(
    ...
    if (result.exitCode !== 0) {
      ...
      if (
        result.exitCode === 1 &&
        (errorMessage.includes("not found") || errorMessage.includes("Image"))
      ) {
    ...
    try {
      ...
      throw new Error(
        `Failed to parse JSON output: ${e}. Output: ${jsonOutput}`,
      );
    }
  }
  ...
  private static getScriptContent(): string {
    ...
    if (!scriptContent) {
      throw new Error(
        "get-oci-image-annotations.py not found in shared scripts",
      );
    }
