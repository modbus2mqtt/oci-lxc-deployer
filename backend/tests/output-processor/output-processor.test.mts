...
describe("OutputProcessor", () => {
  it("should process multiple outputs from JSON array correctly", () => {
    ...
    if (!outputs.has("template_path")) {
      expect.fail(
        `template_path not found in outputs. Available keys: ${outputKeys.join(", ")}, size: ${outputSize}, entries: ${outputEntries}`,
      );
    }
    if (!outputs.has("ostype")) {
      expect.fail(
        `ostype not found in outputs. Available keys: ${outputKeys.join(", ")}, size: ${outputSize}, entries: ${outputEntries}`,
      );
    }
    expect(outputs.get("template_path")).toBe(
