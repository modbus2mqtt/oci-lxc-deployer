...
describe("TemplateProcessor duplicate validation", () => {
  ...
  const veContext = {
    ...
    current: false,
    getStorageContext: () =>
      PersistenceManager.getInstance().getContextManager() as any,
  ...
  it("should detect duplicate templates in the same task", async () => {
    ...
    persistenceHelper.writeJsonSync(
      ...
      "test-duplicate-app/application.json",
      applicationJson,
    );
    ...
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      "test-duplicate-app/templates/template-a.json",
      templateA,
    );
    ...
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      "test-duplicate-app/templates/template-b.json",
      templateB,
    );
  ...
  it("should detect duplicate output IDs from different templates in the same task", async () => {
    ...
    const applicationJson = {
      ...
      installation: { post_start: ["set-db-params-a.json", "set-db-params-b.json"] },
    };
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      "test-duplicate-app/application.json",
    ...
    try {
      ...
      const dbUserError = errorMessages.find(
        (msg: string) =>
          msg.includes("db_user") && msg.includes("set by multiple templates"),
      );
      const dbPasswordError = errorMessages.find(
        (msg: string) =>
          msg.includes("db_password") &&
          msg.includes("set by multiple templates"),
      );
      const dbNameError = errorMessages.find(
        (msg: string) =>
