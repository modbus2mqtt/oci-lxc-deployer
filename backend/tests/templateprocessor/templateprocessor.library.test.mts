...
describe("TemplateProcessor - Library support", () => {
  ...
  it("should error when library file not found", async () => {
    ...
    const applicationJson = {
      ...
      installation: { post_start: ["test-template.json"] },
    };
    ...
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      `${appId}/templates/test-template.json`,
      template,
    );
    ...
    try {
      ...
      const allErrors = [
        errorMessage,
        ...errorDetails.map((d: any) => d.message || String(d)),
      ].join(" ");
  ...
  it("should successfully load template with valid library", async () => {
    ...
    const result = await tp.loadApplication(
      appId,
      "installation",
      veContext,
      ExecutionMode.TEST,
