...
describe("TemplateProcessor skip_if_all_missing", () => {
  ...
  beforeAll(() => {
    ...
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      "test-skip-app/application.json",
      applicationJson,
    );
    ...
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      "test-skip-app/templates/optional-template-single.json",
      optionalTemplateSingle,
    );
    ...
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      "test-skip-app/templates/optional-template-multiple.json",
  ...
  it("should NOT skip template with single parameter when parameter is present", async () => {
    ...
    const skippedCommand = loaded.commands.find(
      (cmd: any) =>
        cmd.name &&
        cmd.name.includes("Optional Template Single") &&
    ...
    const regularCommand = loaded.commands.find(
      (cmd: any) =>
  ...
  it("should skip template with single parameter when parameter is missing", async () => {
    ...
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      "test-skip-app-2/application.json",
      applicationJson2,
    );
    ...
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      "test-skip-app-2/templates/set-parameters-no-output.json",
      setParametersTemplateNoOutput,
    );
    ...
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      "test-skip-app-2/templates/optional-template-single.json",
      optionalTemplateSingle,
    );
  ...
  it("should NOT skip template with multiple parameters when at least one is present", async () => {
    ...
    const applicationJson = {
      ...
      description:
        "Test application for skip_if_all_missing - at least one present",
      installation: { post_start: ["set-parameters.json", "optional-template-multiple.json"] },
    };
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      `${appId}/application.json`,
      applicationJson,
    );
    ...
    const skippedCommand = loaded.commands.find(
      (cmd: any) =>
        cmd.name &&
        cmd.name.includes("Optional Template Multiple") &&
        cmd.name.includes("(skipped)"),
    );
    ...
    expect(regularCommand).toBeDefined();
    expect(regularCommand?.command).toBe(
      "echo 'test command multiple executed'",
    );
  });
});
