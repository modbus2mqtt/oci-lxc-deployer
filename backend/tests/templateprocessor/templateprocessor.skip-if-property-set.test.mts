...
describe("TemplateProcessor skip_if_property_set", () => {
  ...
  beforeAll(() => {
    ...
    const templatesDir = persistenceHelper.resolve(
      ...
      "test-skip-property-set-app/templates",
    );
    ...
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      "test-skip-property-set-app/application.json",
      applicationJson,
    );
    ...
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      "test-skip-property-set-app/templates/set-parameters.json",
    ...
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      "test-skip-property-set-app/templates/skip-if-property-set-template.json",
  ...
  it("should NOT skip template when skip_if_property_set variable is NOT set", async () => {
    ...
    const templatesDir2 = persistenceHelper.resolve(
      Volume.JsonApplications,
      "test-skip-property-set-app-2/templates",
    );
    ...
    const applicationJson2 = {
      ...
      description:
        "Test application for skip_if_property_set - variable not set",
      installation: {
        post_start: [
    ...
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      "test-skip-property-set-app-2/application.json",
      applicationJson2,
    );
    ...
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      "test-skip-property-set-app-2/templates/set-parameters-no-myvariable.json",
    ...
    persistenceHelper.writeJsonSync(
      Volume.JsonApplications,
      "test-skip-property-set-app-2/templates/skip-if-property-set-template.json",
      skipIfPropertySetTemplate,
    );
    ...
    const skippedCommand = loaded.commands.find(
      (cmd: any) =>
        cmd.name &&
        cmd.name.includes("Skip If Property Set Template") &&
