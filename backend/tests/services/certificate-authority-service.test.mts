...
describe("CertificateAuthorityService", () => {
  ...
  describe("generateCA()", () => {
    it("should generate valid CA key and cert", async () => {
      const ca = await service.generateCA(veContextKey);
    ...
    it("should store CA encrypted in context", async () => {
      await service.generateCA(veContextKey);
      expect(await service.hasCA(veContextKey)).toBe(true);
    });

    it("CA cert should be self-signed", async () => {
      await service.generateCA(veContextKey);
      const info = await service.getCaInfo(veContextKey);
  ...
  describe("ensureCA()", () => {
    it("should generate CA if not exists", async () => {
      expect(await service.hasCA(veContextKey)).toBe(false);
      const ca = await service.ensureCA(veContextKey);
      expect(ca.key).toBeTruthy();
      expect(ca.cert).toBeTruthy();
      expect(await service.hasCA(veContextKey)).toBe(true);
    });

    it("should return existing CA if already exists", async () => {
      const ca1 = await service.generateCA(veContextKey);
      const ca2 = await service.ensureCA(veContextKey);
      expect(ca1.key).toBe(ca2.key);
      expect(ca1.cert).toBe(ca2.cert);
    });

    it("should return same CA on repeated calls", async () => {
      const ca1 = await service.ensureCA(veContextKey);
      const ca2 = await service.ensureCA(veContextKey);
      expect(ca1.key).toBe(ca2.key);
      expect(ca1.cert).toBe(ca2.cert);
    });
  ...
  describe("setCA() / getCA()", () => {
    it("should store and retrieve CA", async () => {
      // First generate a CA to get valid key+cert
      const generated = await service.generateCA("ve_temp");
      await service.setCA(veContextKey, generated.key, generated.cert);

      const retrieved = await service.getCA(veContextKey);
      expect(retrieved).not.toBeNull();
      expect(retrieved!.key).toBe(generated.key);
      expect(retrieved!.cert).toBe(generated.cert);
    });

    it("should return null for non-existent VE key", async () => {
      const ca = await service.getCA("ve_nonexistent");
      expect(ca).toBeNull();
    });
  });

  describe("validateCaPem()", () => {
    it("should accept valid PEM key+cert", async () => {
      const ca = await service.generateCA(veContextKey);
      const result = await service.validateCaPem(ca.key, ca.cert);
    ...
    it("should reject invalid PEM format", async () => {
      ...
      const result = await service.validateCaPem(invalidKey, invalidCert);
      expect(result.valid).toBe(false);
      expect(result.error).toBeTruthy();
    });

    it("should reject mismatched key+cert pair", async () => {
      // Generate two different CAs
      const ca1 = await service.generateCA("ve_host1");
      const ca2 = await service.generateCA("ve_host2");
      const result = await service.validateCaPem(ca1.key, ca2.cert);
      expect(result.valid).toBe(false);
  ...
  describe("getCaInfo()", () => {
    it("should return exists=false when no CA", async () => {
      const info = await service.getCaInfo(veContextKey);
      expect(info.exists).toBe(false);
      expect(info.subject).toBeUndefined();
    });

    it("should return subject and expiry when CA exists", async () => {
      await service.generateCA(veContextKey);
      const info = await service.getCaInfo(veContextKey);
      expect(info.exists).toBe(true);
  ...
  describe("generateSelfSignedCert()", () => {
    it("should generate CA-signed server cert", async () => {
      const cert = await service.generateSelfSignedCert(veContextKey, "myhost");
    ...
    it("should auto-generate CA if none exists", async () => {
      expect(await service.hasCA(veContextKey)).toBe(false);
      await service.generateSelfSignedCert(veContextKey, "myhost");
      // ensureCA is called internally, so CA should now exist
      expect(await service.hasCA(veContextKey)).toBe(true);
    });

    it("cert subject should contain hostname", async () => {
      const generated = await service.generateSelfSignedCert(veContextKey, "myhost");
      const subject = readCertSubject(generated.cert);
      expect(subject).toContain("myhost");
    });

    it("each call signs fresh material — server certs are not persisted", async () => {
      ...
      // key+cert material.
      const first = await service.generateSelfSignedCert(veContextKey, "stable-host");
      const second = await service.generateSelfSignedCert(veContextKey, "stable-host");
      expect(second.key).not.toBe(first.key);
      expect(second.cert).not.toBe(first.cert);
    });

    it("does not write a stored cert into the context manager", async () => {
      await service.generateSelfSignedCert(veContextKey, "no-store-host");
  ...
  describe("ensureServerCert()", () => {
    it("delegates to generateSelfSignedCert (no caching, fresh material per call)", async () => {
      const cert = await service.ensureServerCert(veContextKey, "ensure-host");
      expect(cert.key).toBeTruthy();
      expect(cert.cert).toBeTruthy();
    ...
    it("returns different material on repeated calls", async () => {
      const first = await service.ensureServerCert(veContextKey, "repeat-host");
      const second = await service.ensureServerCert(veContextKey, "repeat-host");
      expect(second.key).not.toBe(first.key);
      expect(second.cert).not.toBe(first.cert);
    });
  });
  ...
  describe("ssl_additional_san support", () => {
    it("normalizeExtraSans() handles strings, DNS: prefix, dedup and sort", async () => {
    ...
    it("includes extraSans in the issued cert", async () => {
      const cert = await service.generateSelfSignedCert(
        veContextKey,
        "registry.host",
    ...
    it("ensureServerCert reflects updated extraSans on the next call", async () => {
      const first = await service.ensureServerCert(veContextKey, "san-host", ["a.example"]);
      const sanFirst = readSanExtension(first.cert);
      expect(sanFirst).toContain("a.example");

      const second = await service.ensureServerCert(veContextKey, "san-host", [
        "a.example",
        "b.example",
      ]);
    ...
    it("ensureServerCert SAN list is order-independent (normalizeExtraSans)", async () => {
      const first = await service.ensureServerCert(veContextKey, "stable-host", [
        "DNS:b.example",
        "DNS:a.example",
      ]);
      const second = await service.ensureServerCert(veContextKey, "stable-host", [
        "a.example",
        "b.example",
      ]);
    ...
    it("DNS: prefix is accepted and stripped (matches application.json format)", async () => {
      const cert = await service.generateSelfSignedCert(
        veContextKey,
        "prefix-host",
