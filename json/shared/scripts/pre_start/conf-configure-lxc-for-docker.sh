...
# Allow all devices - simplest and most reliable for Docker
...
# Mount cgroup with read-write access (required for Docker)
echo "lxc.mount.auto = cgroup:rw" >> "$CONFIG_FILE"

# Enable nesting and keyctl features (required for Docker)
echo "Enabling nesting and keyctl features..." >&2
pct set "$VMID" --features nesting=1,keyctl=1 >&2 || {
  echo "Warning: Failed to set nesting features via pct. You may need to add 'features: nesting=1,keyctl=1' manually." >&2
}
