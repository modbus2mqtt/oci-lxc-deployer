...
# on the same L2 segment as the deployer it will replace), with a static IP
# derived from the source by incrementing the last octet by 1 plus an
# offset based on the wall clock to reduce collisions when previous
# clones leaked. The proxvex base image carries no DHCP client (deployer
# CTs always run with host-managed=1 + static IP), so DHCP would just
# leave the clone without a default route.
...
[ -n "$SRC_IPCIDR" ] || fail "Source net0 has no static IP (got: '$SRC_NET0'). Need static IP to derive clone IP."

# Compute clone IP: source last-octet + offset (1..49 wrap to skip .0/.1/etc).
SRC_IP=${SRC_IPCIDR%/*}
SRC_PREFIX=${SRC_IPCIDR#*/}
[ "$SRC_IP" = "$SRC_IPCIDR" ] && SRC_PREFIX="24"
SRC_NET3=${SRC_IP%.*}
SRC_LAST=${SRC_IP##*.}
# Pick offset 1..9 by ts mod, skipping 0/255 boundaries. If source is .250
# we'd overflow so we wrap into .101..109 range; the source-CT lock blocks
# overlapping self-upgrades, so multiple offsets only matter for retries
# after a stale clone leak.
_offset=$(( ( $(date +%s) % 9 ) + 1 ))
...
pct set "$TARGET_VMID" --net0 "$_net0_args" >&2 \
  || fail "pct set --net0 (static, host-managed=1) failed"
